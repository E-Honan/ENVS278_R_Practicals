...
1.  Collate and format our data from the field, including our focal watches and count data (group)
2.  Set up our R project (individual)
3.  Test each dyad in our group using Cohen's Kappa analysis (group)
4.  Make our first visualisations of our count data (individual)

------------------------------------------------------------------------

# Task 1: Formatting your data from the field
...
# Task 2: Setting up your workspace
...
-   For the sub-directory, select somewhere you will remember (like “My Documents” or “Desktop”). **If you save it to your M drive, you will only be able to access it when using a VPN or university computer.**
...
```{r, echo = FALSE, out.width = "50%"}
knitr::include_graphics("photos/example_file_structure.png")
```


## Opening your first R script

-  An R script is a plain text file that contains a series of R commands you can run. 
-  An R script is saved with the extension '.R'.

To create a new script, go to File \> New file \> R Script.

-  Press save, and give the script a sensible title, like "ENVS278_Gulls_Prac_1". Save it to the 'scripts' folder in your R project folder.
...
The syntax for loading multiple libraries is not the same as installing multiple packages. If you're comfortable with R, and would like to try loading multiple libraries at once, the code for this is in the [Optional Extras](#optional-extras) section in next week's practical.
...
knitr::include_graphics("photos/example_script.png")
```
...
## Task 3: Testing for observer bias using Cohen's Kappa
